...
function create_controller($controller_name) {
  ...
  if (!file_exists($controller_filename)) { throw new Exception("le dossier n'existe pas"); }
  include $controller_filename;
  if (!class_exists($controller_classname)) { throw new Exception("Cette classe n'existe pas"); }
  return new $controller_classname();
}

function call_controller_method($controller, $method_name, $parameters) {
 $reflectionObject = new ReflectionObject($controller);
  if (! $reflectionObject->hasMethod($method_name)) { throw new Exception("le nom de cette methode n'existe pas"); }
  $reflectionMethod = $reflectionObject->getMethod($method_name);
  if ($reflectionMethod->getNumberOfParameters()!=count($parameters)){ throw new Exception("le nombre de parametres est trop grand"); }
